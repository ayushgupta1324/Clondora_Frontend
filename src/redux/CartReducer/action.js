...
export const productdata=()=>async(dispatch)=>{
    ...
    const res=await axios.get("https://gentle-tan-starfish.cyclic.app/cart",{
        headers:{
            Authorization:token
        }
    })
...
export const AddProductData=(product)=>async(dispatch)=>{
    const token=localStorage.getItem("token")
    const res=await axios.post("https://gentle-tan-starfish.cyclic.app/cart/create",product,{
        headers:{
...
export const PatchProductData=(el)=>async(dispatch)=>{
    const token=localStorage.getItem("token")
    const res=await axios.patch(`https://gentle-tan-starfish.cyclic.app/cart/${el._id}`,{quantity:Number(el.quantity)+1},{
        headers:{
            "Content-Type": "application/json",
        Authorization: token,
        }
...
export const DeleteProductData=(id)=>async(dispatch)=>{
    const token=localStorage.getItem("token")
    const res=await axios.delete(`https://gentle-tan-starfish.cyclic.app/cart/${id}`,{
        headers:{
            Authorization:token
        }
