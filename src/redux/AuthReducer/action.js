...
export const userRegister=(formstate)=>async(dispatch)=>{
    const res=await axios.post("https://gentle-tan-starfish.cyclic.app/user/register",formstate)
    return dispatch({type:Register,payload:res.data})

}

export const userLogin=(formstate)=>async(dispatch)=>{
    const res=await axios.post("https://gentle-tan-starfish.cyclic.app/user/login",formstate)
